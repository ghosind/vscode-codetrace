...
export class BlameProvider {
    ...
    async getBlameForLine(
        ...
        if (!this.canServe(document)) { return undefined; }

        const cached = this.cache.get(document.uri.fsPath, lineNumber);
        if (cached) { return this.cachedToBlameResult(cached); }
    ...
    async getBlameForVisibleRange(
        ...
        if (!this.canServe(document)) { return resultMap; }

        const uncachedLines = this.collectCachedLines(document, visibleRange, resultMap);
        if (uncachedLines.length === 0) { return resultMap; }
    ...
    private collectCachedLines(
        ...
        for (let line = range.start.line; line <= range.end.line; line++) {
            ...
            if (cached) { resultMap.set(line, this.cachedToBlameResult(cached)); }
            else { uncached.push(line); }
        }
        return uncached;
    }
    ...
    wake(): void { this.isSleeping = false; this.resetIdleTimer(); }
    sleep(): void { this.isSleeping = true; this.cache.clearAll(); }
    reportActivity(): void { if (!this.isSleeping) { this.resetIdleTimer(); } }

    /** Invalidate all cached blame entries for a file. */
    invalidateFile(filePath: string): void { this.cache.invalidateFile(filePath); }

    private resetIdleTimer(): void {
        if (this.idleTimer) { clearTimeout(this.idleTimer); }
        const config = getConfig();
        if (config.idleSleep.enabled) {
            this.idleTimer = setTimeout(() => { this.sleep(); }, config.idleSleep.timeout * 1000);
        }
    }
