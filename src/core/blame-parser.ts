...
export function parseBlamePorcelain(output: string): BlameResult[] {
	...
	for (const line of lines) {
		if (processHeaderLine(line)) { continue; }
		if (!commitProcessed) {
			parseAttr(line);
		}
	}
	...
	function getCachedAttrs(hash: string): Attrs {
		...
		if (!a) { a = { author: '', email: '', timestamp: '', summary: '' }; attrCache.set(hash, a); }
		return a;
	}

	function processHeaderLine(line: string): boolean {
		const fm = line.match(/^([0-9a-f]{40})\s+(\d+)\s+(\d+)\s+(\d+)/);
		if (fm) { handleFullHeader(fm[1], parseInt(fm[3]), parseInt(fm[4])); return true; }
		const cm = line.match(/^([0-9a-f]{40})\s+(\d+)\s+(\d+)$/);
		if (cm) { handleContHeader(cm[1], parseInt(cm[3])); return true; }
		return false;
	}
	...
	function flushCurrent(): void {
		if (!current.hash || commitProcessed) { return; }
	...
	function parseAttr(line: string): void {
		...
		const set = (k: keyof Attrs, v: string): void => {
			...
			if (cached) { cached[k] = v; }
		};
		if (line.startsWith('author ')) { set('author', line.substring(7)); }
		else if (line.startsWith('author-mail ')) { set('email', line.substring(12).replace(/[<>]/g, '')); }
		else if (line.startsWith('author-time ')) { set('timestamp', new Date(parseInt(line.substring(12), 10) * 1000).toISOString()); }
		else if (line.startsWith('summary ')) { set('summary', line.substring(8)); }
	}
}
