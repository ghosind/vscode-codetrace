...
async function initSubsystems(context: vscode.ExtensionContext, repoPath: string): Promise<void> {
    ...
    // Wire inline blame to status bar: show current line's commit hash
    inlineBlameManager.onBlameUpdate = (hash: string | undefined): void => { statusBarManager?.setCurrentLineHash(hash); };
}
...
function registerCommands(context: vscode.ExtensionContext): void {
    const cmds: vscode.Disposable[] = [
        ...
        vscode.commands.registerCommand('codetrace.showFileHistory', async () => {
            ...
            if (e && fileHistoryProvider) { await fileHistoryProvider.refresh(e.document.uri.fsPath); }
        ...
        vscode.commands.registerCommand('codetrace.revertFileVersion', async (hash?: string) => {
            ...
            if (!editor || !gitEngine || !hash) { return; }
        ...
        vscode.commands.registerCommand('codetrace.copyHash', (hash?: string) => {
            if (hash) { vscode.env.clipboard.writeText(hash); }
        }),
    ];
    for (const d of cmds) { context.subscriptions.push(d); allDisposables.push(d); }
}
...
function triggerInitialRefresh(): void {
    const editor = vscode.window.activeTextEditor;
    if (editor) {
        if (fileHistoryProvider) { fileHistoryProvider.refresh(editor.document.uri.fsPath); }
        if (lineHistoryProvider) { lineHistoryProvider.setFile(editor.document.uri.fsPath, editor.selection.active.line); }
    }
}
