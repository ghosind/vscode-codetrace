...
function emptyHtml(key: string): string { return `<div class="empty-state">${t(key)}</div>`; }
...
export class LineHistoryProvider implements vscode.WebviewViewProvider {
    ...
    async refresh(lineNumber: number): Promise<void> {
        if (!this.view || !this.engine || !this.currentFile) { return; }
        this.showLoading();
    ...
    private showEmpty(key: string): void {
        if (this.view) { this.view.webview.html = wrapHtml(t('codetrace.view.lineHistory'), emptyHtml(key)); }
    }
    private showLoading(): void {
        if (this.view) { this.view.webview.html = wrapHtml(t('codetrace.view.lineHistory'), emptyHtml('codetrace.sidebar.loading')); }
    }
