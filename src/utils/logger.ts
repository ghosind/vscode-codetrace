...
export function debug(message: string, data?: unknown): void {
	if (channel && shouldLog('debug')) { channel.appendLine(formatMsg(`DEBUG ${message}`, data)); }
}

/** Info-level log. */
export function info(message: string, data?: unknown): void {
	if (channel && shouldLog('info')) { channel.appendLine(formatMsg(`INFO  ${message}`, data)); }
}

/** Warning-level log. */
export function warn(message: string, data?: unknown): void {
	if (channel && shouldLog('warn')) { channel.appendLine(formatMsg(`WARN  ${message}`, data)); }
}

/** Error-level log (always shown). */
export function error(message: string, data?: unknown): void {
	if (channel) { channel.appendLine(formatMsg(`ERROR ${message}`, data)); }
}
